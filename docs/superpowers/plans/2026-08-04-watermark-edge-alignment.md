...
**Architecture:** Keep the existing app-shell CSS pseudo-element implementation. Strengthen the visual contract first, then change the horizontal offsets from the shared inset variable to `0` and apply the visual-QA gradient refinement; validate the published app video in Chromium.
...
- [ ] **Step 2: Capture and inspect a screenshot**
...
Confirm both masks touch their horizontal media edges, both Doubao marks remain unreadable, the soft fade has no hard rectangular boundary or pale tab, and app UI remains unobstructed.
